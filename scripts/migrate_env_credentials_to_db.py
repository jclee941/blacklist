...
class CredentialMigrator:
    ...
    def migrate_credential(
        ...
        try:
            ...
            if source.lower() not in ["regtech"]:
...
def main() -> int:
    """Main entry point."""
    parser = argparse.ArgumentParser(description="Migrate environment variable credentials to encrypted DB storage")
    ...
    if not any([regtech_id, regtech_pw]):
        parser.print_help()
        return 1
