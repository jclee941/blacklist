...
import { authenticatedPost, loginViaApi } from './auth.fixtures';
...
test.describe('Batch Operations', () => {
  ...
  test.describe('Batch Add API', () => {
    ...
    test('빈 IP 목록으로 일괄 추가 시 에러', async ({ request }) => {
      const response = await authenticatedPost(request, '/api/blacklist/batch/add', { ips: [] });
  ...
  test.describe('Batch Remove API', () => {
    test('IP 목록 일괄 삭제', async ({ request }) => {
      const response = await authenticatedPost(request, '/api/blacklist/batch/remove', {
        ips: ['10.99.99.1', '10.99.99.2'],
      });
    ...
    test('빈 목록으로 일괄 삭제 시 에러', async ({ request }) => {
      const response = await authenticatedPost(request, '/api/blacklist/batch/remove', { ips: [] });
  ...
  test.describe('Batch Update API', () => {
    ...
    test('빈 업데이트 요청 시 에러', async ({ request }) => {
      const response = await authenticatedPost(request, '/api/blacklist/batch/update', { ips: [] });
