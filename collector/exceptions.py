...
class CredentialNotFoundError(CredentialError):
    ...
    def __init__(self, source: str):
        self.source = source
        super().__init__(
            f"{source.upper()} credentials not found in database. "
            f"Please add credentials via API: POST /api/credentials"
        )
...
class CredentialDecryptionError(CredentialError):
    ...
    def __init__(self, source: str, original_error: Exception = None):
        ...
        super().__init__(
            f"Failed to decrypt {source.upper()} credentials: {str(original_error)}"
        )
...
class MissingMasterKeyError(CredentialError):
    ...
    def __init__(self):
        super().__init__(
            ...
            "Cannot decrypt credentials from database."
        )
